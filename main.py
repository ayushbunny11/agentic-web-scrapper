...
class WebScrapingAgent:
    ...
    def _extract_text_content(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        for i in range(1, 7):
            ...
            if headings:
                text_content["headings"][f"h{i}"] = [h.get_text().strip() for h in headings]
        
        # Extract paragraphs
        paragraphs = soup.find_all('p')
        text_content["paragraphs"] = [p.get_text().strip() for p in paragraphs if p.get_text().strip()]
...
def main():
    ...
    for url in test_urls:
        ...
        if "error" not in data:
            ...
            print(f"Text paragraphs: {len(data['text_content']['paragraphs'])}")
            ...
            if data['text_content']['paragraphs']:
                print(f"First paragraph: {data['text_content']['paragraphs'][0][:100]}...")
